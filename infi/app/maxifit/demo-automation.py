...
# --- CONFIGURATION ---
SHORTCUT_PATH = r"C:\Users\Administrator\Desktop\MaxiFitPointV5.05.lnk"

# Compute dynamic config path (or take it from command line argument if provided)
BASE_DIR = Path(__file__).resolve().parent.parent
default_config = BASE_DIR / "document_extraction" / "Mie Tsu_extracted.json"
...
def handle_save_dialog(filename):
    ...
    if savewin:
        ...
        try:
            ...
            # vk_packet=True sends raw unicode characters, bypassing IME/keyboard layout issues
            savewin.type_keys(save_path, with_spaces=True)
            time.sleep(0.3)
...
def main():
    ...
    # STAGE 1: Location
    print("\n[1] Location")
    if not set_combo(main_win, "areaSelectComboBox", location["area"]): return
    time.sleep(0.5)
    if not set_combo(main_win, "pointSelectComboBox", location["point"]): return
    time.sleep(0.5)
    
    # STAGE 2: Add First PCS
    print("\n[2] Adding First PCS")
    first_array = CONFIG['pcs_config'][0]
    print(f"  PCS: {first_array['pcs_type']}")
    
    if not set_combo(main_win, "pcsSelectComboBox", first_array['pcs_type']): return
    ...
    for array_idx, array_config in enumerate(CONFIG['pcs_config']):
        print(f"\n  === Array Type {array_idx + 1} ===")
        print(f"    PCS: {array_config['pcs_type']}")
        print(f"    Panel: {array_config['module_type']}")
        print(f"    Series: {array_config['modules_per_string']}, Parallel: {array_config['strings']}")
        print(f"    Angle: {array_config['tilt']}°, azimuth: {array_config['azimuth']}°")
        print(f"    Total Arrays: {array_config['pcs_count']}")
        
        # For array types after the first, add new PCS
        if array_idx > 0:
            print(f"  Adding new PCS type...")
            
            if not set_combo(main_win, "pcsSelectComboBox", array_config['pcs_type']): 
        ...
        if popup.exists():
            popup.set_focus()
            set_combo(popup, "panelSelectComboBoxSub", array_config['module_type'])
            set_combo(popup, "panelSeriesNumericUpDown", array_config['modules_per_string'], True)
            set_combo(popup, "panelParallelNumericUpDown", array_config['strings'], True)
            set_combo(popup, "installationAngleNumericUpDown", array_config['tilt'], True)
            set_combo(popup, "azimuthNumericUpDown", array_config['azimuth'], True)
        ...
        num_copies = array_config['pcs_count'] - 1
        if num_copies > 0:
