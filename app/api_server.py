...
INTEGRATOR_SCRIPT = BASE_DIR / "integrated_simulator.py"
...
async def run_pipeline(
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        # We run it exactly as in the terminal
        process = subprocess.run(
            [sys.executable, "-X", "utf8","-u", str(INTEGRATOR_SCRIPT), "--pdf_path", pdf_path, "--user_inputs_json", inputs_path],
            cwd=str(BASE_DIR),
            #capture_output=True,
            #text=True,
            encoding="utf-8" ) #Make the API read UTF-8
